...
poligonos <- "VMVS_CUAD_979.shp"
name <- str_remove(poligonos,".shp")

#  *****CAMBIAR EL WORK DIRECTORY****    ruta del shp a trabajar
...
tab_merge <- merge(poly_rev, tab_corresp, by.x = "LC_2016", by.y = "clase_samof", all=FALSE) # False = solo se pegan valores existentes TRUE = pega todo
head(tab_merge)

#para identificar cambios != lc_16 
poly_rev2 <- mutate(tabl_merge, dif_16_c = case_when(Val_Cambio == 'C' & Val_T1 != val_ipcc & is.na(C_Anio)  ~ 1 , TRUE ~ 0))
names(poly_rev2$dif_16_c)
head(tabl_merge)
...
#Condicionantes despues del join con "clase_samof"  para obtener el "val_ipcc
